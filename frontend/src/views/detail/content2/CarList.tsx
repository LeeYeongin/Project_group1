        ...
        <ul className="car5_title" onClick={() => setMenu(isOpen => !isOpen)} key={section}><h4>강의 {section}</h4>
            {itemList.map((item) => (
                <li className={isOpen ? "car5_item" : "hide_item"}>
                    강의 {item.part}
                    <iframe className={idUser ? (status ? (isOpen ? "car5_video" : "hide_video") : "hide_video") : "hide_video"} width="560" height="315" src={item.video} title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen></iframe>
                </li>
            ))}
        </ul>
    )
    ...
    return(
        <div className="car5_list">
            {sectionList.map((section) => (
                <CarDetail status={status} idUser={idUser} section={section} curriItems={curriItems} />
            ))}
        </div>
    );
}
