...
function Main5() {
  ...
  const getClassDetailHandler = async (token: string) => {
    ...
    await axios
      .get('http://localhost:4040/api/orderlist', requestOption)
      .then((response) => {
  ...
  const onScrollClick = (id: string) => {
    if (id === 'content1')
      idRef1.current?.scrollIntoView({ behavior: 'smooth' });
    if (id === 'content2')
      idRef2.current?.scrollIntoView({ behavior: 'smooth' });
    if (id === 'content3')
      idRef3.current?.scrollIntoView({ behavior: 'smooth' });
  ...
  const ScrollBtn = btnList.map((BTN) => {
    return (
      <button
        key={BTN.id}
        className={`${select === BTN.divId ? 'menu5 view' : 'menu5'}`}
        type="button"
        onClick={() => onScrollClick(BTN.divId)}
      >
        {BTN.divTitle}
      </button>
    );
  ...
  const putCart = () => {
    ...
    // id와 함께 장바구니로 넘어감
    axios
      .post('http://localhost:4040/api/cart/add', addCart, requestOption)
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4040/api/main5/${idClass}/`)
      .then((response) => {
    ...
    <>
      {detailItems && instruct && (
        <div className="detail5_main" key={detailItems.idClass}>
          ...
          <div
            className={ScrollActive ? 'detail5_menu5 fixed' : 'detail5_menu5'}
          >
          ...
          <div className="detail5_content">
            <div className="detail5_content_main">
              <div
                id="content1"
                className={ScrollActive ? 'con5 scroll' : 'con5'}
                ref={idRef1}
              >
                <h1>강의소개</h1>
                <div className="explan">
                  ...
                  <div className="instruct5">
                    <img
                      className="instruct_img"
                      src={instruct[0].instructorImg}
                    />
              ...
              <div id="content2" className="con5" ref={idRef2}>
                ...
                <Carlist
                  status={status}
                  idUser={idUser}
                  curriItems={detailItems.curriculumList}
                />
              </div>
              <div id="content3" className="con5" ref={idRef3}>
                ...
                <ReviewList
                  idUser={idUser}
                  idClass={idClass}
                  reviewItems={detailItems.reviewList}
                />
              </div>
            </div>
            {/* 장바구니 */}
            <div className="detail5_payment">
              <div className="pay_body5">
                ...
                <button
                  id="login_backet"
                  className={status ? 'disabled' : 'backet5'}
                  onClick={putCart}
                >
                  장바구니
                </button>
                <button
                  id="login_backet"
                  className={status ? 'backet5' : 'disabled'}
                  onClick={moveMyPage}
                >
                  마이페이지로 이동
                </button>
