...
export default function CartList({
  ...
  const deleteHandler = () => {
    axios
      .post('http://localhost:4040/api/cart/delete', checkValue, requestOption)
      .then((Response) => {
  ...
  return (
    <>
      <div className="cart-control2">
        ...
        <button className="cancel-control2" onClick={() => deleteHandler()}>
          선택삭제 X
        </button>
      </div>
      {itemList.map((item) => (
        <div className="cart-list2">
          <input
            ...
            onChange={(e) => {
              cartCheckHandler(e.currentTarget.value);
            }}
          ...
          <div className="payment2">{item.price}원</div>
        </div>
      ))}
    </>
  );
